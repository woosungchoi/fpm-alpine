...
GitHub Actions is the sole publisher for PHP 8.2–8.5. Docker Hub Automatic Builds and the legacy publication webhook have been removed. Manual releases use the owner-only typed `fpm-manual-publish` `repository_dispatch`, so GitHub always loads `publish.yml` from protected default-branch `main`; eligible dependency-only changes on protected `main` use a separate unattended controller that cannot pass its mutation preflight until the current Docker Hub and GHCR subjects have semantic baseline parity. Before automatic Docker Hub credentials and again immediately before moving mutation, the controller requires the exact source to remain current `main`, requires the permanent-cutover repository variables, and anonymously verifies the live Docker Hub repository identity, active status, and `is_automated=false`; any intervening `last_updated` drift fails closed. Source pull requests cannot access registry credentials or publish images.
