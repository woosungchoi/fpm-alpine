...
- Production promotion requires one explicit PHP minor per dispatch, downloaded and content-validated GHCR evidence from two consecutive canary runs, repository variable `LEGACY_PUBLISHER_DISABLED=true`, and explicit dispatch input `legacy_publisher_disabled=true`. Manual production additionally requires a matching fresh 15-minute cutover-evidence hash. Automatic production instead requires a completed successful `dependency-auto-promote` run bound to the exact current `main` SHA, an affected-minor authorization artifact, durable quiescent cutover evidence at least 24 hours old, and a live Docker Hub `is_automated=false` read-back before every mutation stage.
