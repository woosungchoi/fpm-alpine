...
- `published-runtime-smoke` resolves each active Docker Hub moving tag once and carries the captured immutable digest through manifest, provenance, SBOM, runtime, and vulnerability verification weekly and after the Docker-Hub-only dependency publisher runs. After a successful manual production publisher run, it additionally resolves GHCR immutably and verifies Cosign signatures and cross-registry semantic parity.
