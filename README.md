...
>
> For production use, **pin an explicit image tag / version branch** such as `woosungchoi/fpm-alpine:8.3` instead of relying on `latest`.
...
`bash
./scripts/check-manifest.sh woosungchoi/fpm-alpine:8.3
`
