...
GitHub Actions is the sole publisher for PHP 8.2–8.5. Docker Hub Automatic Builds and the legacy publication webhook have been removed. An explicitly enabled trusted-main controller may promote strictly eligible dependency merges after two verified immutable canaries; source pull requests cannot access registry credentials or publish images.
...
- Production promotion requires one explicit PHP minor per dispatch, downloaded and content-validated GHCR evidence from two consecutive canary runs, repository variable `LEGACY_PUBLISHER_DISABLED=true`, and explicit dispatch input `legacy_publisher_disabled=true`. Manual production additionally requires a matching fresh 15-minute cutover-evidence hash. Automatic production instead requires a completed successful `dependency-auto-promote` run bound to the exact current `main` SHA, an affected-minor authorization artifact, a durable `automation/conveyor-lock` ref, and the newest owner-dispatched immutable cutover-lease artifact captured within 15 minutes. Both privileged mutation stages recheck current `main`, the conveyor lock, and that lease after environment approval.
...
`dependency-update-pr` is a separate, disabled-by-default updater. Once the repository-scoped GitHub App and `DEPENDENCY_AUTOMATION_ENABLED=true` are configured, it may open one isolated pull request at a time for official PHP same-minor patch/digest updates and PECL patch updates. It cannot merge, publish, or modify lifecycle/runtime policy. `dependency-auto-merge` independently revalidates only the oldest allowlisted PR's exact diff and GitHub Actions App-bound `docker-smoke` result before requesting native auto-merge. `dependency-auto-promote` may dispatch two exact full-matrix canaries only when explicitly enabled. With `DEPENDENCY_AUTO_PRODUCTION_ENABLED=true`, the separate trusted `dependency-auto-production` workflow sequentially promotes only the authorized affected minors, dispatches the next candidate only after successful production read-back, and freezes on any failure.
