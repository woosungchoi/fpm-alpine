...
- `dependency-auto-publish`
  ...
  - A single `fpm-auto-production` controller downloads and revalidates the exact active artifact set before using registry credentials. Automatic mode requires the source SHA to remain the current protected `main` and requires `LEGACY_PUBLISHER_DISABLED=true` plus `DOCKERHUB_TAG_POLICY_ENFORCED=true`. Before Docker Hub credentials and immediately before moving mutation it anonymously reads the live Docker Hub repository metadata, requires the canonical repository to be active with `is_automated=false`, records the safe metadata as transaction evidence, and rejects any intervening `last_updated` drift. No owner dispatch or short-lived lease is required for this automatic path. The manual publisher retains its separate owner-attested lease contract. This environment is a protected-main unattended authority boundary, not a reviewer-approval or environment-secret isolation claim.
...
3. Require the protected-branch-only `fpm-auto-production` unattended environment and valid repository-scoped Docker Hub credentials before enabling publication. Confirm that the automatic controller's two anonymous Docker Hub metadata checks report the canonical active repository with `is_automated=false` and stable `last_updated`. Do not describe the environment as a reviewer gate unless reviewers are actually configured; the manual publisher retains the separate owner-attested lease.
