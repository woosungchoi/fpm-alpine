...
- `dependency-auto-publish`
  - It runs automatically only when protected `main` changes `build/versions.json` and the merged diff is accepted by the dependency classifier. A failed trusted automatic publication may be retried with the typed owner-only `fpm-dependency-publish-replay` dispatch; the requested source must be an exact ancestor whose `build/versions.json` blob is byte-for-byte identical to current trusted `main`. The image is rebuilt from a separate checkout of that exact source while the current default-branch workflow supplies the verifier and journal, and the request rejoins the same `automatic` mutation path. Backfill is not a branch-selectable `workflow_dispatch`; it is the typed `fpm-ghcr-backfill` `repository_dispatch`, so GitHub loads the workflow definition from the default branch. Its payload must name `backfill-ghcr` and an exact ancestor source SHA, and that commit's `build/versions.json` must exactly equal the trusted default-branch release manifest before the active matrix is accepted.
...
# Owner-only replay of a failed trusted automatic publication. The source versions blob must
# still exactly match current main; the current default-branch workflow remains authoritative.
HOME=/home/openclaw XDG_CONFIG_HOME= gh api --method POST repos/woosungchoi/fpm-alpine/dispatches \
  -f event_type=fpm-dependency-publish-replay \
  -F 'client_payload[operation]=automatic-replay' \
  -F 'client_payload[source_sha]=<40-hex-failed-source-sha>'
