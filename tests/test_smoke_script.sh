...
case "${1:-}" in
  run)
    if [ "${MOCK_FAIL_RUN_CONTAINS:-}" ] && [[ "$*" == *"$MOCK_FAIL_RUN_CONTAINS"* ]]; then exit 1; fi
    if [[ " $* " == *" -d "* ]]; then printf '%s\n' mock-container; fi
    ;;
  inspect)
...
run=[c for c in calls if c[0]=='run']; assert len(run)==2
for call in run:
    assert '--platform' in call and call[call.index('--platform')+1]==platform
    assert 'fixture:'+platform.split('/')[1] in call
